...
; Function to clean everything and set the initial conditions
to setup
  clear-all
  setup-payoffs
  setup-players
...
; Funtion to read the payoffs matrix and extract the number of strategies
to setup-payoffs
...
; Function to:
;      1. read the number of players for each strategy
;      2. check if the numero of players is mach with the payoff matrix
;      3. check if the sum of number of players is the same of numer of patches
;      4. initialize the patches-varable
to setup-players
...
to setup-graph
  ...
  foreach (range n-of-strategies) [ i ->
    ...
    set-plot-pen-mode 1
    set-plot-pen-color 25 + 10 * i    ; 10 40
  ]

end


; Funtion to count the number of agents and their strategy and do the plot
to update-graph
...
; Function to set the colors on plot
to update-color
  set pcolor 25 + 10 * strategy
end



; Function to statr the simulation of model
to go
  ask patches [play]
  ask patches [
...
; Function to define the payoff based on payoff matrix
to play
  ifelse play-only-with-neighborhood
      [ set mate one-of other patches ]
      [ set mate one-of other my-coplayers ]
  set payoff item ([strategy] of mate) (item strategy payoff-matrix)

end

; Function to update the startegy based on value of noise
to update-strategy
  ...
  [
    ...
    ifelse play-only-with-neighborhood
      [ set observed-player one-of other patches ]
      [ set observed-player one-of other my-coplayers ]
